...
pub(crate) trait Variable {
    ...
    async fn get<'a, T>(
        &self,
        gbl_fb: &mut GblFastboot<'a, '_, '_, '_, impl GblOps<'a>, impl BufferPool, T>,
        name: &str,
        args: Split<'_, char>,
        out: &mut [u8],
...
impl Variable for (&'static str, &'static str) {
    async fn get<'a, T>(
        &self,
        _: &mut GblFastboot<'a, '_, '_, '_, impl GblOps<'a>, impl BufferPool, T>,
        name: &str,
        _: Split<'_, char>,
        out: &mut [u8],
    ) -> Result<Option<usize>, CommandError> {
...
macro_rules! fb_vars_api {
    ($($vars:expr),+ $(,)?) => {
        ...
        ///
        ///   pub(crate) async fn fb_vars_get<'a, T>(
        ...
        ///
        ///   pub(crate) async fn fb_vars_get_all<'a, T>(
        ///       gbl_fb: &mut GblFastboot<...>,
        ///       responder: &mut impl VarInfoSender,
        ///   ) -> Result<(), CommandError> {
